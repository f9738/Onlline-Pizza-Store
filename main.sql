...
ACCEPT X1 PROMPT 'ENTER THE MENU ITEMS ID ONE BY ONE SEPARATED BY COMMA (MAXIMUM NUMBER OF ITEMS IS 5) :   '
ACCEPT Y1 PROMPT 'ENTER THE QUANTITY OF THE ITEMS ONE BY ONE SEPARATED BY COMMA  :   '
ACCEPT CN PROMPT 'NAME OF THE CUSTOMER  :   '
ACCEPT CA PROMPT 'ADDRESS OF THE CUSTOMER  :   '
...
CREATE OR REPLACE PACKAGE ONIGIRI AS 

	FUNCTION ORDERSS (MENU_IDS IN CUSTOMERS.CUSTOMER_ADDRESS%TYPE, QUANTITIES IN CUSTOMERS.CUSTOMER_ADDRESS%TYPE, CN IN CUSTOMERS.CUSTOMER_NAME%TYPE, CA IN CUSTOMERS.CUSTOMER_ADDRESS%TYPE)
	RETURN NUMBER;

END ONIGIRI;
/

CREATE OR REPLACE PACKAGE BODY ONIGIRI AS 
	...
	BEGIN
		...
		IF(M1!='0' AND M2!='0' AND M3!='0' AND M4!='0' AND M5!='0') THEN
			INSERT INTO ORDERS (O_ID, m_ID,  O_ITEMS, QUANTITY, PRICE, CUSTOMER_NAME, CUSTOMER_ADDRESS, L_ID, THE_DATE) VALUES (N1+1, M1,  ''||(SELECT M_NAME_SIZE FROM MENU WHERE M_ID = M1)||'', Q1, Q1*(SELECT M_PRICE FROM MENU WHERE M_ID = M1) ,''||CN||'',''||CA||'',1,SYSDATE);    
			INSERT INTO ORDERS (O_ID, m_ID,  O_ITEMS, QUANTITY, PRICE, CUSTOMER_NAME, CUSTOMER_ADDRESS, L_ID, THE_DATE) VALUES (N1+1, M2,  ''||(SELECT M_NAME_SIZE FROM MENU WHERE M_ID = M2)||'', Q2, Q2*(SELECT M_PRICE FROM MENU WHERE M_ID = M2) ,''||CN||'',''||CA||'',1,SYSDATE);    
			INSERT INTO ORDERS (O_ID, m_ID,  O_ITEMS, QUANTITY, PRICE, CUSTOMER_NAME, CUSTOMER_ADDRESS, L_ID, THE_DATE) VALUES (N1+1, M3,  ''||(SELECT M_NAME_SIZE FROM MENU WHERE M_ID = M3)||'', Q3, Q3*(SELECT M_PRICE FROM MENU WHERE M_ID = M3) ,''||CN||'',''||CA||'',1,SYSDATE);    
			INSERT INTO ORDERS (O_ID, m_ID,  O_ITEMS, QUANTITY, PRICE, CUSTOMER_NAME, CUSTOMER_ADDRESS, L_ID, THE_DATE) VALUES (N1+1, M4,  ''||(SELECT M_NAME_SIZE FROM MENU WHERE M_ID = M4)||'', Q4, Q4*(SELECT M_PRICE FROM MENU WHERE M_ID = M4) ,''||CN||'',''||CA||'',1,SYSDATE);    
			INSERT INTO ORDERS (O_ID, m_ID,  O_ITEMS, QUANTITY, PRICE, CUSTOMER_NAME, CUSTOMER_ADDRESS, L_ID,THE_DATE) VALUES (N1+1, M5,  ''||(SELECT M_NAME_SIZE FROM MENU WHERE M_ID = M5)||'', Q5, Q5*(SELECT M_PRICE FROM MENU WHERE M_ID = M5) ,''||CN||'',''||CA||'',1,SYSDATE); 
...
BEGIN
	...
	ORDERTAKINGG := ONIGIRI.ORDERSS(M_IDS, QTITIES, CN, CA);
